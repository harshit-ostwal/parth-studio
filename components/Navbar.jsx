...
export default function Navbar() {
    ...
    return (
        <>
            ...
            <div className="fixed w-full h-32 bg-white shadow-lg flex p-7">
            ...
            <div
                //Sidebar CSS
                className={
                    Menu ? "fixed left-0 top-0 w-[100%] sm:w-[70%] md:w-[25%] flex flex-col transition-all duration-1000 ease-in-out p-8 h-full bg-slate-800 text-slate-400" : "fixed left-[-100%] w-[100%] sm:w-[70%] md:w-[25%] top-0 flex flex-col transition-all duration-1000 ease-in-out p-8 h-full bg-slate-800 text-slate-400"
                }>
                ...
                <div className="flex flex-1 flex-col justify-start mt-20 items-center gap-4">
                    {SidebarData.map((Data) => (
                        <Link className="font-Merienda text-2xl font-semibold" onClick={Toggle} key={Data.id} href={Data.href}>{Data.title}</Link>
                    ))}
                </div>

                {/* Sidebar Socail Media Icons */}
